...
<body>
    ...
    <h1 class="categories-header">Телефоны</h3>


    <?php
    include ('assets/scripts/array_paginator.php');
    $url = '/cursed/phones.php'; 
    $paginator = new ArrayPaginator($url, 4); 

    $sql = "SELECT * FROM Products where idCategory = 3";
 ...
 <option value="1" <?php echo $selectedFilter == '1' ? 'selected' : ''; ?>>Без фильтров</option>
    ...
    <script>
        ...
        document.getElementById('shop_filters').addEventListener('change', function () {
            ...
            if (selectedValue) {
                const url = `phones.php?selected=${encodeURIComponent(selectedValue)}`;
    ...
    if ($_SESSION['selected_filter'] == 2) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 3 order by ProductPrice ASC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $data = $array;
        $paginator->setItems($data);
    ...
    if ($_SESSION['selected_filter'] == 3) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 3 order by ProductPrice DESC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $paginator->setItems($array);
        $items = $paginator->getItems();
    ...
    if ($_SESSION['selected_filter'] == 4) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 3 order by ProductName ASC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $paginator->setItems($array);
        $items = $paginator->getItems();
    ...
    if ($_SESSION['selected_filter'] == 5) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 3 order by ProductName DESC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $paginator->setItems($array);
        $items = $paginator->getItems();

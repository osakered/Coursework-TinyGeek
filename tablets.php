...
<body>
    ...
    <h1 class="categories-header">Планшеты</h3>


    <?php
    include ('assets/scripts/array_paginator.php');
    $url = '/cursed/tablets.php'; 
    $paginator = new ArrayPaginator($url, 4); 

    $sql = "SELECT * FROM Products where idCategory = 4";
    ...
    <script>
        document.getElementById('shop_filters').addEventListener('change', function () {
            ...
            if (selectedValue) {
                const url = `tablets.php?selected=${encodeURIComponent(selectedValue)}`;
    ...
    if ($_SESSION['selected_filter'] == 2) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 4 order by ProductPrice ASC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $data = $array;
        $paginator->setItems($data);
    ...
    if ($_SESSION['selected_filter'] == 3) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 4 order by ProductPrice DESC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $paginator->setItems($array);
        $items = $paginator->getItems();
    ...
    if ($_SESSION['selected_filter'] == 4) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 4 order by ProductName ASC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $paginator->setItems($array);
        $items = $paginator->getItems();
    ...
    if ($_SESSION['selected_filter'] == 5) {
        $sql = "SELECT * FROM `Products` WHERE idCategory = 4 order by ProductName DESC";
        $result = $link->query($sql);
        $array = mysqli_fetch_all($result, MYSQLI_ASSOC);
        $paginator->setItems($array);
        $items = $paginator->getItems();
